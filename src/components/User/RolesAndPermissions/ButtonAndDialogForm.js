...
export default function RolesAndPermissionButtonAndDialogForm({ data: user, isLoading: isDataLoading }) {
	...
	const { data: roles } = useSWR('/data/roles', url => axios.get(url).then(response => response.data));
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			formData.forEach((value, key) => key === 'roles' ? roles.push(value) : null);
				...
				!isDataLoading && <Dialog fullWidth
						...
						<Box sx={{
							...
							>
								<FormControl
									...
									variant="standard"
								>
									<FormLabel component="legend">Hak Akses</FormLabel>
									<FormGroup>
										{
											roles?.map(role => <div key={role.id}>
												<FormControlLabel
													label={role.name}
													control={<>
														<Checkbox name="roles" value={role.name} defaultChecked={user.role_names.includes(role.name)} />
													</>
													}
												/>
												<FormHelperText>{role.permissions.map(perm => perm.name).join(', ')}</FormHelperText>
											</div>
											)
										}
									</FormGroup>
...

					{
						!isComplete && <DialogActions>
							<Button color="info" disabled={isLoading} type='reset' onClick={handleClose}>Batal</Button>
							<Button color="info" disabled={isLoading} variant="contained" form="set_roles_and_permission" type="submit" >Simpan</Button>
						</DialogActions>
					}
